...
public class Main {
                ...
                switch (choice) {
                    ...
                    case "2":
                        try {
                            ...
                            System.out.println("图片已生成并保存为 graph.png");
                    ...
                    case "3":
                        ...
                        if (bridgeWords.isEmpty()) {
                            ...
                            System.out.println("The bridge words from \"" + word1 + "\" to \"" + word2 + "\" is: \"" + singleWord + "\"");
                        } else {
                            String bridgeWordsStr = bridgeWords.stream()
                                    .sorted()
                                    .collect(Collectors.joining(", "));
                            System.out.println("The bridge words from \"" + word1 + "\" to \"" + word2 + "\" are: \"" + bridgeWordsStr + "\"");
                        }
                        break;
                    case "4":
                    ...
                    case "7":
                        ...
                        Thread walkThread = new Thread(() -> {
                            ...
                            // 保存结果到文件
                            String filename = "random_walk_" + System.currentTimeMillis() + ".txt";
    ...
    // 生成图片并返回 BufferedImage
    public static BufferedImage generateDotImage(Map<String, Map<String, Integer>> graph) throws IOException, InterruptedException {
        // DOT 文件头部
            ...
            if (part.matches("[a-zA-Z]+")) {
                ...
                if (prevWasWord && prevWord != null) {
                    ...
                    if (bridges != null && !bridges.isEmpty()) {
                        Random random = new Random(); // 正确初始化Random对象
                        int index = random.nextInt(bridges.size());
    ...
    public static String calcShortestPath(String start, String end) {
            ...
            Map<String, String> previous = new HashMap<>();   // 存储每个节点的前驱节点
    ...
    public static Double calPageRank(String word) {
        ...
        graph.keySet().forEach(node -> finalPr.put(node, initPR));
            ...
            for (String u : graph.keySet()) {
                ...
                for (String v : graph.keySet()) {
                    if (graph.get(v).containsKey(u)) {
                        incoming += pr.get(v) / graph.get(v).size();
                    }
                }
